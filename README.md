# João Flávio Pedroso do Carmo

**Software Engineering · Backend-focused · Building foundations**

---

I'm a software engineering student at UFC Russas, currently focused on backend development.
I study consistently, care about writing clean and organized code, and I'm building my path toward becoming a solid software engineer.

---

## About Me

- 🎓 Software Engineering — Universidade Federal do Ceará (UFC Russas)
- 💻 Currently focused on **backend development** with Java
- 🔍 Interested in **APIs, software architecture, and backend systems**
- 📈 Long-term goal: grow into a full stack developer with strong backend foundations
- 🤝 Active member and **Marketing Coordinator** at [Include Jr]([https://github.com](https://www.linkedin.com/company/include-jr/posts/?feedView=all)) — Junior Enterprise of UFC Russas

---

## Skills

### Currently studying
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)

### Have worked with 
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

---

## Roadmap

What I'm working toward in the near future:

```
[✓] Logic and problem-solving with Python
[✓] Git basics and version control practices
[~] Java fundamentals and OOP               ← in progress
[ ] Spring Boot and REST APIs
[ ] Relational databases (SQL)
[ ] System design basics
[ ] First backend project deployed
```

---

## Include Jr — Junior Enterprise · UFC Russas

I'm part of **Include Jr**, the Junior Enterprise of UFC Russas, where I currently serve as **Marketing Coordinator**.

In this role, I develop strategic communication, organizational, and leadership skills — managing campaigns, coordinating with different teams, and contributing to the visibility of real technical projects.

I'm also **joining technology projects within the enterprise**, which is allowing me to apply my development studies in a practical, team-oriented environment for the first time.

> Being part of a Junior Enterprise has given me early exposure to professional dynamics, deadlines, and collaborative work — experience I carry into every project I approach.

---

## Currently learning from

- University coursework (UFC Russas — Computer Science)
- Personal study: Java, Git, backend concepts
- Include Jr projects and team experience

---

## Contact

| | |
|---|---|
| 📧 Email | joaoflavioes@gmail.com |
| 💼 LinkedIn | https://www.linkedin.com/in/joao-flavio-es/ |
| 🐙 GitHub | You're already here :) |

---

<sub>Consistently learning. Honestly representing where I am. Always improving.</sub>
